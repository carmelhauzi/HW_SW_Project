...
# script_raytrace.sh
#
# End-to-end performance harness for the "raytrace" benchmark. Intended to
# be run inside the QEMU environment, from the repository root:
#
#   ./script_raytrace.sh [normal|fast|rigorous]
#
# It will:
...
# benchmarks/raytrace/optimized/run_benchmark.py to get fresh numbers.
...
run_variant() {
  ...
  python3 "$script_path" ${PYPERF_FLAG} \
    ...
    --filename "${RESULTS_DIR}/render_${variant}.ppm" \
